...
class SqlGenerator {
  ...
  static Future<bool> deleteColumna(
      {required String add,
      required sql.Database database,
      required String nombreDB}) async {
    List<Map<String, dynamic>> columnas =
    ...
    for (Map<String, dynamic> columna in columnas) {
      String name = columna['name'];
      if (name == add) {
        debugPrint('eliminando $add');
        await database.execute('ALTER TABLE $nombreDB DROP COLUMN $add');
        return true;
      }
    }
    debugPrint('No se encontro $add para eliminar');
    return false;
  }
