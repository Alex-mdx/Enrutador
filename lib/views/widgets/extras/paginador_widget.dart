...
class _PaginadorGroupedWidgetState extends State<PaginadorGroupedWidget> {
  ...
  Widget build(BuildContext context) {
    debugPrint("${widget.max} - ${widget.length} ");
    var max =
        (((widget.length >= widget.maxLenght ? (widget.max) : widget.length) ==
                        0
                    ? 1
          ...
          children: [
            if (widget.itemScrollController != null)
              IconButton.filledTonal(
                  ...
                  icon:
                      Icon(LineIcons.arrowCircleUp, color: ThemaMain.primary)),
            IconButton.filledTonal(
                iconSize: 19.sp,
                onPressed: () async {
                  if (index != 1) {
                ...
                icon: Icon(LineIcons.angleDoubleLeft, color: ThemaMain.green)),
            IconButton.filledTonal(
                iconSize: 19.sp,
                onPressed: () async {
                  if (index != 1) {
            ...
            IconButton.filledTonal(
                ...
                icon: Icon(LineIcons.angleDoubleRight, color: ThemaMain.green)),
            if (widget.itemScrollController != null)
              IconButton.filledTonal(
                  iconSize: 19.sp,
                  onPressed: () async => await widget.itemScrollController!
                      .scrollTo(
                          ...
                          curve: Curves.easeInOut),
                  icon:
                      Icon(LineIcons.arrowCircleDown, color: ThemaMain.primary))
          ]),
